...
class TeamState:
    ...
    # Publishers (will be set after creation)
    task_status_publisher: object = None
...
option_list = [
    ...
    #     name="Assemble",
    #     id=0,
    #     description="This is used to get into formation.",
    #     example_code="node.assemble(robots=['burger1', 'tb4', 'robot3'])"
    # ),
    # TestOption(
    #     name="AssembleAt",
    #     id=1,
    #     description="This is used to assemble at a particular location",
    #     example_code="node.assemble_at(goal='round table', robots=['burger1', 'tb4', 'robot3')"
    # ),
    TestOption(
        name="Goto",
        id=2,
        description="This is used to goto only after robots are already in formation",
        example_code="node.goto(goal='fridge', robotts=['burger1', 'tb4', 'robot3')"
    )
]
...
class TeamLLMNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # ========== DYNAMIC PUBLISHERS ==========
        self.team_publishers: dict[str, object] = {}  # team_name -> Publisher
        # ========================================
    ...
    def create_team_publisher(self, team_name: str):
        """Create a dedicated publisher for this team's status"""
        if team_name not in self.team_publishers:
            topic = f'/{team_name}_task_status'
            self.team_publishers[team_name] = self.create_publisher(String, topic, 10)
            self.get_logger().info(f"Created publisher for {team_name}: {topic}")
        return self.team_publishers[team_name]
    ...
    def get_or_create_team(self, team_name: str, robots: list[str]) -> TeamState:
        ...
        with self.team_lock:
            if team_name not in self.teams:
                ...
                team.task_status_publisher = self.create_team_publisher(team_name)
    ...
    def cleanup_team(self, team_name: str):
        ...
        with self.team_lock:
            if team_name in self.teams:
                ...
                # Keep publisher for potential future use
                # (could also remove if memory is concern)
    ...
    def update_team_status(self, team_name: str, status: str, task: str = None):
        """Update team status and publish to all relevant topics"""
        with self.team_lock:
            if team_name not in self.teams:
                return
        ...
        if task:
            status_msg = f"{team_name.upper()} (status): {task} - {status}"
        else:
            status_msg = f"{team_name.upper()} (status): {status}"

        msg = String()
        msg.data = status_msg

        # Publish to global task status
        self.pub_task_status.publish(msg)

        # Publish to team-specific topic
        if team_name in self.team_publishers:
            self.team_publishers[team_name].publish(msg)

        # Log to team history
        self.log_team_history(team_name, status_msg)
    ...
    def on_tasks_json(self, msg: String) -> None:
        ...
        try:
            ...
            for key, task_str in robot_tasks.items():
                ...
                # Parse tasks (could be comma-separated)
                tasks = [t.strip() for t in task_str.split(',') if t.strip()]
                team_assignments[team_name]["tasks"].extend(tasks)
    ...
    def handle_team_assignment(self, team_name: str, robots: list[str], tasks: list[str]):
        ...
        # Get or create team
        team = self.get_or_create_team(team_name, robots)
    ...
    def execute_team_task(self, team_name: str, task: str):
        ...
        prompt = (
            f"You are controlling a team of robots named '{team_name}' with members: {team.robots}.\n"
            f"Generate Python code to perform the task using available actions.\n\n"
            f"Robot Context:\n{robot_context}\n\n"
            f"Team State:\n{json.dumps(team_state, indent=2)}\n\n"
            f"Recent History:\n{chat_history}\n\n"
            f"Available Actions:\n{available_actions}\n\n"
            f"IMPORTANT: Use 'node' to refer to the TeamLLMNode instance.\n"
            f"The team_name '{team_name}' will be automatically passed to functions.\n"
        )
        ...
        if code:
            ...
            if explanation:
                self.get_logger().info(f"[{team_name}] Explanation: {explanation}")
    ...
    def assemble(self, robots: str = None) -> bool:
        """Team assembles at computed centroid"""
        if not robots:
            raise ValueError("team_name is required")
        
        team = self.teams.get(robots)
        if not team:
            return False
        
        self.get_logger().info(f"[{robots}] Assembling robots: {team.robots}")
        
        # Update states for all robots
        for robot in team.robots:
            self.update_team_robot_states(robots, robot, {
                "action": "assembling",
                "formation": "active"
            })
        
        # TODO: Actual assembly logic (publish to navigation manager)
    ...
    def assemble_at(self, goal: str, robots: str = None) -> bool:
        ...
        self.get_logger().info(f"[{team_name}] Assembling at {goal}: {team.robots}")
    ...
    def goto(self, goal: str, team_name: str = None) -> bool:
        ...
        self.get_logger().info(f"[{team_name}] Going to {goal}: {team.robots}")
