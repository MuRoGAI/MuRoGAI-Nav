#!/usr/bin/env python3
"""
ROS2 Path Planner Node - UPGRADED VERSION
Supports:
...
- Request cancellation when new request arrives
"""
...
class PathPlannerNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("Path Planner Node starting (UPGRADED)...")
        self.get_logger().info("="*60)
        ...
        self.offsets = {
            ...
            "robot4": (16.0, 8.0, 0.0, 3.14),
            "robot5": (16.0, 10.0, 0.0, 3.14),
            "robot6": (15.0, 9.0, 0.0, 3.14),
        }
    ...
    def path_plan_callback(self, msg: PathPlannerRequest):
        ...
        if self._planning_active:
            ...
            self.get_logger().warn(f"New request #{self._current_request_id} received - cancelling current planning")
            self.get_logger().warn("="*60)
            self._cancel_current_planning = True
            # Give a brief moment for the cancellation flag to be processed
            time.sleep(0.1)
        ...
        if self._cancel_current_planning:
            ...
            self.get_logger().warn(f"✗ Request #{self._current_request_id} cancelled")
        elif success:
    ...
    def _plan_paths(self, plan_json: dict) -> bool:
        ...
        try:
            ...
            if self._cancel_current_planning:
                self.get_logger().warn("Planning cancelled during initialization")
                return False
            ...
            if self._cancel_current_planning:
                self.get_logger().warn("Planning cancelled after parsing")
                return False
            ...
            for agent_entry in agents_list:
                if self._cancel_current_planning:
                    self.get_logger().warn("Planning cancelled - new request received")
                    return False
            ...
            if all_results:
                ...
                # Run all post-processing (save/visualize) in a single background thread
                # This prevents blocking the main thread from accepting new requests
                def post_process():
                    try:
                        ...
                        if self.save_image:
                            self._save_path_image_thread_safe(all_results, plt_thread)
                        
                        # Save video if enabled
                        if self.save_video:
                            self._save_path_video_thread_safe(all_results, plt_thread)
                        
                        # Visualize if enabled - skip in background thread, use main thread instead
                        if self.show_path_visualization:
                            self.get_logger().info("Skipping interactive visualization in background thread")
                            self.get_logger().info("(Set show_path_visualization=False if not needed)")
                    except Exception as e:
                        self.get_logger().error(f"Post-processing error: {e}")
                        traceback.print_exc()
                
                # Start post-processing in background
                post_process_thread = threading.Thread(target=post_process, daemon=True, name="PostProcessing")
                post_process_thread.start()
                self.get_logger().info("Post-processing started in background thread")
    ...
    def _parse_formation_group(self, group_name: str, group_data: dict) -> Optional[dict]:
        ...
        try:
            ...
            desired_radius = float(group_data.get('desired_radius', 1.0))
    ...
    def _get_robot_pose(self, robot_name: str) -> dict:
        ...
        if robot_name in self.robot_current_poses:
            ...
            return pose_data
            # else:
            #     self.get_logger().warn(f"Odom for {robot_name} is stale ({age:.1f}s old)")
    ...
    def _plan_single_agent(self, name: str, agent, start: np.ndarray, goal: np.ndarray,
        ...
        try:
            ...
            if self._cancel_current_planning:
                self.get_logger().warn(f"Planning cancelled before starting for {name}")
                return None
            
            # Run planning in a thread so we can check for cancellation
            result_container = {'traj': None, 'exception': None, 'finished': False}
            
            def run_planner():
                try:
                    traj = planner.plan(start, goal, dynamic_obstacles)
                    result_container['traj'] = traj
                except Exception as e:
                    result_container['exception'] = e
                finally:
                    result_container['finished'] = True
            
            planning_thread = threading.Thread(target=run_planner, daemon=True)
            ...
            while not result_container['finished']:
                if self._cancel_current_planning:
                    self.get_logger().warn(f"Planning cancelled during execution for {name}")
                    # Note: We can't actually stop the planner thread, but we can ignore its result
                    return None
            ...
            traj = result_container['traj']
            
            # Final check after planning completes
            if self._cancel_current_planning:
                self.get_logger().warn(f"Planning cancelled after completion for {name}")
                return None
    ...
    def _save_path_video(self, all_results: list):
        ...
        try:
            ...
            # 10 fps is enough for smooth playback, cap at 300 frames max
            fps = 10
            duration = max(max_time, 3.0)  # At least 3 seconds
            num_frames = min(int(duration * fps), 300)  # Cap at 300 frames
            
            self.get_logger().info(f"Video config: {num_frames} frames at {fps} fps, duration={num_frames/fps:.1f}s, trajectory_time={max_time:.1f}s")
            
            # Create figure - use non-interactive backend
            plt.ioff()
            fig, ax = plt.subplots(figsize=(10, 8), dpi=80)  # Lower DPI for faster rendering
            ...
            self.get_logger().info(f"Rendering frames...")
            ...
            def animate(frame):
                ...
                # Track which arrows need to be removed and recreated
                arrows_to_update = []
                ...
                for elem in robot_elements:
                    ...
                    if np.any(mask):
                        ...
                        # Calculate current position and theta
                        if current_time >= t_data[-1]:
                            ...
                            if idx > 0 and idx < len(t_data):
                                ...
                                if t1 > t0:
                                    ...
                                    # Interpolate theta for diff-drive
                        ...
                        # Update direction arrow for diff-drive
            ...
            # Create animation without blit for better compatibility
    ...
    def _save_path_video_thread_safe(self, all_results: list, plt_module):
        """Thread-safe version of _save_path_video using Agg backend"""
        
        try:
            self.get_logger().info("Creating path animation video (background thread)...")
            
            # Find the most recent run directory
            run_dir = self._get_latest_run_dir()
            video_path = run_dir / "planned_paths.mp4"
            ...
            plt_module.ioff()
            fig, ax = plt_module.subplots(figsize=(10, 8), dpi=80)
            ...
            for traj_data in all_traj_data:
                ...
                for robot_idx in range(num_robots):
                    ...
                    # Get radius
                    agent = traj_data['agent']
                    radius = agent.radius if hasattr(agent, 'radius') else 0.2
            ...
            def animate(frame):
                ...
                for elem in robot_elements:
                    ...
                    if len(t_data) == 0 or current_time < t_data[0]:
                        elem['circle'].set_visible(False)
                        elem['text'].set_visible(False)
                        elem['line'].set_data([], [])
                        if elem['arrow'] is not None:
            ...
            self.get_logger().info("Video generation complete (background thread)")
            
        except Exception as e:
            self.get_logger().error(f'Failed to create path video (thread): {e}')
            traceback.print_exc()
    ...
    def _save_path_image(self, all_results: list):
        ...
        try:
            ...
            self._plot_paths_on_axis(ax, all_results)
    ...
    def _save_path_image_thread_safe(self, all_results: list, plt_module):
        """Thread-safe version of _save_path_image using provided pyplot module"""
        
        try:
            # Create figure without display
            plt_module.ioff()
            fig, ax = plt_module.subplots(figsize=(12, 10))
    ...
    def _plot_paths_on_axis(self, ax, all_results: list):
        """Plot paths on given matplotlib axis (reusable for both main and background threads)"""
        ax.imshow(
            self.grid[::-1],
            cmap="gray_r",
            extent=[0, self.W * self.resolution, 0, self.H * self.resolution],
    ...
    def _visualize_paths(self, all_results: list):
        """Visualize all planned paths with individual robot positions"""
        ...
        try:
            ...
            # Use colors from agent configs, fallback to default palette
            default_colors = ['tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:purple', 'tab:brown']
            color_map = {
                'blue': 'tab:blue',
                'orange': 'tab:orange', 
            ...
            for idx, result in enumerate(all_results):
                ...
                for robot_idx in range(num_robots):
                    ...
                    # Plot goal marker (FIXED: removed mew parameter)
                    ax.plot(x_vals[-1], y_vals[-1], 'x', color=color, ms=8,
                           markeredgecolor='black', markeredgewidth=2)
            ...
            # Keep figure open for a bit then close it to prevent accumulation
            plt.pause(5.0)  # Show for 5 seconds
            plt.close(fig)
