...
# ============================================================
# NEW Numba-accelerated functions for agent geometry
# ============================================================
...
def _nb_compute_formation_discs(
    ...
    """Compute formation disc positions.
    
    Returns: (Nr, 3) array where each row is [x, y, radius]
    
    Math: p_i = center + R(th) @ diag(sx, sy) @ p_star_i
    """
    Nr = P_star.shape[0]
...
def _nb_compute_robot_poses(
    ...
    """Compute robot poses for formation.
    
    Returns: (Nr, 3) array where each row is [x, y, heading]
    """
    Nr = P_star.shape[0]
    result = np.empty((Nr, 3), dtype=np.float64)
...
def _nb_max_robot_displacement(
    ...
    """Compute max Euclidean displacement between formations."""
    Nr = P_star.shape[0]
...
def _nb_interpolate_formation_5d(
    ...
    """Interpolate between two 5D formation configurations."""
...
def _nb_interpolate_individual_2d(
    ...
    """Interpolate between two 2D individual agent configurations."""
...
def _nb_rs_like_proxy(
    ...
    """Cheap RS-like proxy for a single robot."""
    dx = x2 - x1
    dy = y2 - y1
    c = math.cos(th1)
    s = math.sin(th1)
...
def _nb_max_rs_like_distance(
    ...
    """Max RS-like distance over all robots in formation."""
    Nr = P_star.shape[0]
...
def _nb_formation_nn_distance(
    ...
    """Combined affine + RS-like distance for formations."""
    # Affine term
...
# REPLACE the entire interpolate_q() method WITH:
...
class FlexibleFormationAgent:
    def __init__(
        ...
        self.radius = self.radii.tolist()  # Keep list interface for compatibility
...
# REPLACE the entire discs() method WITH:

    def discs(self, q: np.ndarray) -> List[Tuple[np.ndarray, float]]:
        key = self._qkey(q)
        cached = self._disc_cache.get(key, None)
        if cached is not None:
            # Convert cached array back to list of tuples
    ...
    # REPLACE the entire robot_poses() method WITH:
    ...
    # REPLACE the entire max_robot_displacement() method WITH:
...
# REPLACE the entire max_rs_like_distance() method WITH:

    def max_rs_like_distance(self, q1: np.ndarray, q2: np.ndarray) -> float:
        """Max RS-like proxy over all robots."""
        q1 = np.asarray(q1, dtype=float)
        q2 = np.asarray(q2, dtype=float)
    ...
    # REPLACE the entire dist_for_nn() method WITH:
                ...
                if not dynamic_obstacles:
                    ...
                    for _j in range(max_extend):
                        ...
                        if self._nn_dist(q_cur, q_goal) <= self.d_max + 1e-9:
                            ...
                            q_next, _ = self._steer(q_cur, q_goal)

                            if q_next is not None:
                                if self._edge_static_ok(q_cur, q_next):
                                    # safe
