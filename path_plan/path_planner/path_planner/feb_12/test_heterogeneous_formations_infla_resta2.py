...
# robot_1 = DifferentialDriveAgent(
#     radius=0.35,
...
# robot_2 = HolonomicAgent(
#     radius=0.30,
#     v_max=1.2, a_max=2.5,
#     inflation=INFLATION_RADIUS,
# )

# robot_3 = DifferentialDriveAgent(
#     radius=0.35,
#     v_max=0.8, omega_max=2.5,
#     a_max=2.0, alpha_max=4.0,
#     inflation=INFLATION_RADIUS,
...
waffle = DifferentialDriveAgent(
    radius=0.3,
    v_max=0.26, omega_max=1.82,
    a_max=2.5, alpha_max=3.2,
    inflation=INFLATION_RADIUS,
)

tb4 = DifferentialDriveAgent(
    radius=0.3,
    v_max=0.31, omega_max=1.9,
    a_max=0.9, alpha_max=2.0,
    inflation=INFLATION_RADIUS,
)

burger_team = HeterogeneousFormationAgent(
    P_star=[[-0.57, -0.19], [0.57, -0.19], [0.0, 0.38]],
    robot_types=['diff-drive', 'diff-drive', 'diff-drive'],
    radius=[0.25, 0.25, 0.25],
    
    v_max=[0.22, 0.22, 0.22],           # 4x FASTER (was 0.2, 0.25, 0.2)
    omega_max=[2.84, 2.84, 2.84],       # 3.75x FASTER (was 0.4, 0.0, 0.4)
    a_max=[2.5, 2.5, 2.5],           # 30x FASTER! (was 0.05, 0.4, 0.05)
    alpha_max=[3.2, 3.2, 3.2],       # Slightly increased
    
    sx_range=(0.9, 4.0),            # Limited scaling
    sy_range=(0.9, 4.0),
    inflation=INFLATION_RADIUS,
)
...
agents = [
    ("TeamBurger", burger_team,
     np.array([2.28, 1.33, 1.57, 1.0, 1.0]),
     np.array([2.28, 6.84, 1.57, 1.0, 1.0]),
     "heterogeneous-formation"),

    # ("HeteroForm2", hetero_form2,
    #  np.array([6.33, 16.0, 0.0, 1.0, 1.0]),
    #  np.array([18.0, 8.0, 1.57, 3.82, 2.04]),
    #  "heterogeneous-formation"),

    ("Waffle", waffle,
     np.array([3.42, 6.84, -1.57]),
     np.array([1.14, 2.28, -1.57]),
     "diff-drive"),

    ("TB4", tb4,
     np.array([0.57, 3.99, 0.0]),
     np.array([3.42, 4.56, 0.0]),
     "diff-drive"),


    # ("robobt1", robot_1,
    #  np.array([4.0, 8.0, 0.0]),
    #  np.array([18.0, 12.5, 0.0]),
    #  "diff-drive"),
    ...
    # ("robot3", robot_3,
    #  np.array([20.0, 12.5, 0.0]),
    #  np.array([12.0, 4.0, 0.0]),
    #  "diff-drive"),
...
# Prepare 
#  parameters (use first formation agent)
warmup_kino_params = {
    'robot_types': burger_team.robot_types,
    'v_max': burger_team.v_max_list,
    'w_max': burger_team.omega_max_list,
    'a_max': burger_team.a_max_list,
    'alpha_max': burger_team.alpha_max_list,
    'N_steer': 8,
    'T_steer': 0.8,
}
...
for name, agent, start, goal, agent_type in agents:
    ...
    planner = SIRRT(
        agent_model=agent,
        max_velocity=0.22,                # ⬅️ Match robot speeds
        workspace_bounds=bounds,
        static_grid=static_grid,
        time_horizon=120.0,              # ⬅️ Increased from 100
        max_iter=5000,                   # ⬅️ Increased from 1200
        d_max=0.05,                      # ⬅️ Increased from 0.15
        goal_sample_rate=0.35,           # ⬅️ Increased from 0.30
        neighbor_radius=0.8,             # ⬅️ Increased from 1.8
        precision=2,
        seed=205,
        debug=False,
