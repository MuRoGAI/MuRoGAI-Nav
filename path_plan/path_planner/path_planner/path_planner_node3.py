...
class PathPlannerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('show_path_visualization', False) # add save image, video of the generated path
        ...
        for idx, robot_name in enumerate(self.robot_names):
            ...
            # ---- Initialize using offsets ----
            offset_x, offset_y, offset_yaw = self.offsets.get(robot_name, (0.0, 0.0, 0.0))
        ...
        # Wait briefly for odom messages
    ...
    def _display_initial_map(self):
        ...
        try:
            ...
            ax.set_xlabel("X [m]")
            ax.set_ylabel("Y [m]")
            ax.set_title("Path Planner - Initial Map\n(Solid=Odom, Dashed=Default)")
    ...
    def path_plan_callback(self, msg: PathPlannerRequest):
        ...
        # Check if planning is already active - immediately cancel it
        if self._planning_active:
            self.get_logger().warn("="*60)
            self.get_logger().warn(f"New request #{self._current_request_id} received while planning in progress")
            self.get_logger().warn("Cancelling current planning immediately...")
            self.get_logger().warn("="*60)
            self._cancel_current_planning = True
            # Don't wait - just set the flag and continue with new request
    ...
    def _parse_formation_group(self, group_name: str, group_data: dict) -> Optional[dict]:
        ...
        # Extract robot types and CURRENT positions
        robot_types = []
        robot_positions = []
        robot_names = []
        robot_radii = []
        ...
        for robot_entry in robots:
            ...
            # Get robot CURRENT position from ODOM (for computing start centroid and P_star)
            if robot_name in self.robot_current_poses:
                pose = self.robot_current_poses[robot_name]
                x = pose['x']
                y = pose['y']
                self.get_logger().info(f"    Robot {robot_name} current position: ({x:.3f}, {y:.3f})")
            else:
        ...
        # Compute P_star from current positions using provided function
        ...
        self.get_logger().info(f"    Start centroid (from robot positions): ({centroid_start[0]:.3f}, {centroid_start[1]:.3f})")
        ...
        # Aggregate velocity limits for formation (per-robot lists)
        ...
        for agent_name, traj in agent_paths.items():
            ...
            if agent_type == "heterogeneous-formation":
                ...
                for robot_idx in range(Nr):
                    ...
                    robot_traj_msg.robot_name = robot_name
                    robot_traj_msg.robot_type = robot_type
                    
                    # Create appropriate trajectory message
                    if robot_type == 'diff-drive':
                        ...
                        robot_traj_msg.diff_drive_trajectories.append(traj_msg)
                        ...
                        robot_traj_msg.holo_trajectories.append(traj_msg)
                        
                        self.get_logger().info(
                            f"  - {robot_name} (Holo): {len(all_t)} points, duration={all_t[-1]:.2f}s"
                        )
                    
                    msg.robot_trajectories.append(robot_traj_msg)
                ...
                robot_traj_msg = RobotTrajectory()
                robot_traj_msg.robot_name = robot_name
                robot_traj_msg.robot_type = agent_type
    ...
    def _save_trajectories(self, agent_paths: dict, agent_models: dict, 
                          agent_info: dict, control_trajectories: dict):
        """Save trajectories to disk with auto-versioning"""
        ...
        for agent_name, traj in agent_paths.items():
            ...
            if agent_type == "heterogeneous-formation":
                ...
                saved[agent_name] = {
                    "times": np.array(all_t) if all_t else np.array([]),
                    "robots": robot_trajs
                }
        ...
        try:
            ...
            for agent_name, traj in agent_paths.items():
                ...
                if agent_type == "heterogeneous-formation":
                    ...
                    pts = []
                    for item in traj:
                        q = item[0] if isinstance(item, (list, tuple)) else item
                    ...
                    start_q = start_item[0] if isinstance(start_item, (list, tuple)) else start_item
                    goal_q = goal_item[0] if isinstance(goal_item, (list, tuple)) else goal_item
