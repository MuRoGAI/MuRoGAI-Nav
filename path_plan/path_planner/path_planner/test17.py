...
class RobotConfig:
    ...
    w_max:           float = 2.84
    use_rsr:         bool  = False
    max_pose_offset: float = 2.0  # override per robot; see MAX_POSE_OFFSET
...
ROBOT_CONFIG: dict[str, RobotConfig] = {
    ...
    "firebird": RobotConfig(
        ...
        # firebird's odom_world origin is offset ~6m from CSV start — this is
        # normal for this robot. Set large enough to never trigger the check.
        max_pose_offset=10.0,
    ),
    "tb4_1": RobotConfig(
        csv_file="tb4_1.csv",
...
MAX_POSE_OFFSET   = 2.0   # metres; reject trajectory if initial offset exceeds this
COORD_TIMEOUT     = 10.0  # seconds; proceed even if some robots never check in
ODOM_STALE_WARN   = 0.8   # seconds; log warning but keep controlling
ODOM_STALE_LIMIT  = 2.0   # seconds; stop and publish zero — pose too old to trust
WATCHDOG_PERIOD   = 2.0   # seconds; log error if no cmd published for this long
...
# ---------------------------------------------------------------------------
# Coordinator — with timeout so one slow/crashed robot never blocks the rest
# ---------------------------------------------------------------------------
class Coordinator:
    def __init__(self, robot_names, sync_delay=SYNC_DELAY, timeout=COORD_TIMEOUT):
        ...
        self._first_ready_time = None   # wall-clock of first check-in

    def report_ready(self, robot_name, now_sec):
        with self._lock:
            self._ready.add(robot_name)

            if self._first_ready_time is None:
                self._first_ready_time = now_sec

            all_ready = self._ready >= self._expected
            timed_out = (now_sec - self._first_ready_time) > self._timeout

            if (all_ready or timed_out) and self._start_time is None:
                missing = self._expected - self._ready
                if missing:
                    print(
                        f"[COORD] TIMEOUT after {self._timeout}s — "
                        f"missing robots: {missing}. Proceeding anyway."
                    )
...
# ---------------------------------------------------------------------------
# PathFollower — all reliability fixes applied
# ---------------------------------------------------------------------------
class PathFollower(Node):
    def __init__(self, robot_name, coordinator):
        ...
        # FIX 1: store pending start time so the race window is not missed
        self._pending_start  = None

        # FIX 6: odom staleness tracking
        self._last_odom_time = 0.0

        # FIX 5: watchdog tracking
        self._last_cmd_time     = 0.0
        self._watchdog_warned   = False

        # FIX 6: odom staleness tracking
        self._last_odom_warn_time = 0.0
        ...
        self.get_logger().info(
            f"[INIT] {self.robot}  "
            f"sub_qos=SENSOR_DATA  pub_qos=RELIABLE  "
        ...
        # FIX 5: watchdog timer — separate slow timer, always running
    ...
    def odom_callback(self, msg):
        ...
        self._last_odom_time = self.get_clock().now().nanoseconds * 1e-9  # FIX 6
    ...
    def create_trajectory(self, shared_start_time):
        ...
        # FIX 2: reject if odom pose is implausibly far from CSV start.
        # We allow a few retries in case odom hasn't settled yet, then
        # proceed anyway with a warning rather than blocking forever.
        offset = math.hypot(dx, dy)
        if offset > self.cfg.max_pose_offset:
            self._pose_offset_retries = getattr(self, '_pose_offset_retries', 0) + 1
            if self._pose_offset_retries <= 5:
                self.get_logger().warn(
                    f"[TRAJ] {self.robot}: pose offset ({dx:.2f},{dy:.2f}) = "
                    f"{offset:.2f}m exceeds max_pose_offset={self.cfg.max_pose_offset}m — "
                    f"retry {self._pose_offset_retries}/5, waiting for odom to settle"
                )
                return   # keep _pending_start intact so we retry next tick
            else:
                self.get_logger().error(
                    f"[TRAJ] {self.robot}: pose offset {offset:.2f}m still exceeds "
                    f"max_pose_offset={self.cfg.max_pose_offset}m after 5 retries — "
                    f"proceeding anyway. Check odom or robot starting position."
                )
                # fall through and build trajectory with whatever pose we have
        ...
        if self.cfg.use_rsr:
            ...
            label = (f"{len(self.waypoints)-1} waypoints → "
                     f"{len(self.segments)} sub-segments")
        else:
            ...
            if dropped:
                self.get_logger().warn(
                    ...
                    f"first segment t0={self.segments[0]['t0']:.3f}s. "
                    f"Robot will clamp to first segment immediately (no stall)."
                )
    ...
    def _active_segment(self, t):
        ...
        if t <= seg['tf'] + 0.5:
            return seg
        # Only reachable if t is far past the last segment — control_loop
        # goal-seek logic should have caught this first, but log it clearly.
        self.get_logger().warn(
            f"[SEG] {self.robot}: _active_segment returned None "
            f"t={t:.2f} last_tf={seg['tf']:.2f} seg_idx={self._seg_idx}"
        )
        return None
    ...
    # ------------------------------------------------------------------
    def _watchdog(self):
        """Fires every WATCHDOG_PERIOD seconds. Logs an error if no cmd
        has been published recently despite the trajectory being active."""
        if not self.traj_created or not self.segments:
            return
        now = self.get_clock().now().nanoseconds * 1e-9
        gap = now - self._last_cmd_time
        if gap > WATCHDOG_PERIOD and not self._watchdog_warned:
            self._watchdog_warned = True
            t   = now - self.start_time if self.start_time else -1.0
            seg = self._active_segment(t) if self.segments else None
            seg_str = "None" if seg is None else f"[{seg['t0']:.1f},{seg['tf']:.1f}]"
            self.get_logger().error(
                f"[WATCHDOG] {self.robot}: no cmd_vel published for {gap:.1f}s! "
                f"traj_t={t:.2f}s  seg_idx={self._seg_idx}  seg={seg_str}"
            )
        elif gap <= WATCHDOG_PERIOD:
            # Reset so it can warn again if the robot stalls later
            self._watchdog_warned = False
    ...
    def control_loop(self):
        ...
        # FIX 6: odom staleness — tiered response
        #   < ODOM_STALE_WARN  : normal
        #   < ODOM_STALE_LIMIT : warn but keep controlling (pose still usable)
        #   >= ODOM_STALE_LIMIT: stop and skip — pose too old to trust
        odom_age = now - self._last_odom_time
        if self._last_odom_time > 0 and odom_age > ODOM_STALE_WARN:
            if odom_age > ODOM_STALE_LIMIT:
                self.get_logger().error(
                    f"[ODOM] {self.robot}: odom too stale ({odom_age:.2f}s > "
                    f"{ODOM_STALE_LIMIT}s) — stopping robot"
                )
                self.cmd_pub.publish(Twist())
                return
            else:
                if now - self._last_odom_warn_time > 1.0:
                    self._last_odom_warn_time = now
                    self.get_logger().warn(
                        f"[ODOM] {self.robot}: odom slightly stale ({odom_age:.2f}s) "
                        f"— continuing with last known pose"
                    )

        if self.traj_failed:
            return

        # FIX 1: always save pending start; retry trajectory creation every tick
        if not self.traj_created:
            shared_start = self.coord.report_ready(self.robot, now)
            if shared_start is not None:
                self._pending_start = shared_start   # persist across ticks
            if self._pending_start is not None and self.current_pose is not None:
                self.create_trajectory(self._pending_start)
            return
        ...
        # FIX 4: clamp t forward past any gap caused by dropped segments
        first_t0 = self.segments[0]['t0']
        if t < first_t0:
            t = first_t0
