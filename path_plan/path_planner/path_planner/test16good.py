...
ODOM_STALE_LIMIT  = 0.5   # seconds; stop publishing if odom is this old
...
class PathFollower(Node):
    ...
    def create_trajectory(self, shared_start_time):
        ...
        if offset > MAX_POSE_OFFSET:
            self.get_logger().error(
                ...
                f"{offset:.2f}m exceeds MAX_POSE_OFFSET={MAX_POSE_OFFSET}m — "
                f"odom may be stale or robot was moved. Retrying next tick."
            )
            # Invalidate pending start so coordinator re-evaluates on next tick
            self._pending_start = None
            return
    ...
    def control_loop(self):
        ...
        # FIX 6: odom staleness guard — stop robot and skip control if stale
        odom_age = now - self._last_odom_time
        if self._last_odom_time > 0 and odom_age > ODOM_STALE_LIMIT:
            self.get_logger().warn(
                f"[ODOM] {self.robot}: stale odom ({odom_age:.2f}s > "
                f"{ODOM_STALE_LIMIT}s) — publishing zero cmd and skipping"
            )
