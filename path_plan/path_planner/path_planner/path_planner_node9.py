...
ROS2 Path Planner Node - UPGRADED VERSION with Request Cancellation
Supports:
...
class PathPlannerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('n_steer', 10)
        self.declare_parameter('t_steer', 1.0)
        self.declare_parameter('t_steer_min', 0.5)
        self.declare_parameter('t_steer_safety_factor', 1.5)
        self.declare_parameter('dt', 0.05)
        # self.declare_parameter('seed', 438)
        # self.declare_parameter('seed', 684)
        self.declare_parameter('seed', 93)
    ...
    def path_plan_callback(self, msg: PathPlannerRequest):
        ...
        # Check if there's already a planning thread running
    ...
    def _plan_paths(self, plan_json: dict, request_id: int) -> bool:
        ...
        try:
            ...
            for agent_entry in agents_list:
                ...
                # Plan path (this is the long-running operation)
            ...
            if all_results:
                ...
                # Save outputs (these are quick operations, no cancellation check needed)
                if self.save_paths:
        ...
        try:
            ...
            for robot_name in robots:
                ...
                if 'holonomic' in rtype or 'omnidirectional' in rtype:
                    ...
                    a_max_list.append(robot_cfg.get('max_acceleration', self.default_max_angular_velocity))
                else:
        ...
        try:
            ...
            if 'holonomic' in rtype or 'omnidirectional' in rtype:
                ...
                vel_limits = {
                    ...
                    'v_min': -robot_cfg.get('max_linear_velocity_x', self.default_max_velocity),
        ...
        try:
            ...
            if agent_type == "heterogeneous-formation":
                kino_params = {
                    ...
                    'T_steer_min': self.t_steer_min,
                    'T_steer_safety_factor': self.t_steer_safety_factor,
            ...
            # This is where the long planning happens
            # We cannot interrupt it, but we check cancellation before and after
            traj = planner.plan(start, goal, dynamic_obstacles)
            
            # Check for cancellation immediately after planning
            if self._cancel_event.is_cancelled():
                self.get_logger().warn(f"[Thread-{request_id}] Cancelled after planning {name}")
                return None
