#!/usr/bin/env python3
"""
ROS2 Path Planner Node - UPDATED VERSION (NO THREADING)
...
- Request cancellation using simple variables (no threading)
"""
...
# REMOVED: CancellableEvent class (entire class deleted)
...
class PathPlannerNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("Path Planner Node starting (NO THREADING VERSION)...")
        self.get_logger().info("="*60)
        
        # ============================================================
        # Simple cancellation system (NO THREADING)
        # ============================================================
        ...
        self.map_file_path = self._resolve_file(
            ...
            default_pkg='path_planner',
            default_relative_path='data/restaurant_5.npy'
        )
    ...
    def path_plan_callback(self, msg: PathPlannerRequest):
        """Handle incoming path planning requests - SIMPLIFIED (NO THREADING)"""
        ...
        # Cancel previous request if planning is in progress
        if self._is_planning:
            self.get_logger().warn("="*60)
            self.get_logger().warn(f"Request #{self._current_request_id} is still running")
            self.get_logger().warn(f"Cancelling it for new request #{request_id}")
            self.get_logger().warn("="*60)
            self._cancel_requested = True
            self.stats['cancelled'] += 1
            
            # STORE THE NEW REQUEST TO PROCESS AFTER CANCELLATION
            self._pending_request = (plan_json, request_id)
            self.get_logger().info(f"Request #{request_id} queued as pending")
            return  # ← EXIT - don't reset _cancel_requested or start planning yet!
        
        # No planning in progress, start immediately
        self._execute_planning_request(plan_json, request_id)

    def _execute_planning_request(self, plan_json: dict, request_id: int):
        """Execute a planning request and handle any pending requests afterward"""
        
        # Update current request
        self._current_request_id = request_id
        self._cancel_requested = False
        
        # Start planning directly (NO THREAD)
        self.get_logger().info(f"Starting execution of request #{request_id}")
        result = self._plan_paths(plan_json, request_id)
        ...
        if self._pending_request is not None:
            pending_plan, pending_id = self._pending_request
            self._pending_request = None
            
            self.get_logger().info("="*60)
            self.get_logger().info(f"Processing pending request #{pending_id}")
            self.get_logger().info("="*60)
            
            # Recursively process the pending request
            # (which may itself be cancelled by another pending request)
            self._execute_planning_request(pending_plan, pending_id)
    ...
    def _plan_paths(self, plan_json: dict, request_id: int) -> bool:
        """Plan paths for all robots/formations in the request - SIMPLIFIED (NO THREADING)"""
        
        self._is_planning = True
        
        try:
            self.get_logger().info("="*60)
            self.get_logger().info(f"[Request-{request_id}] Parsing plan JSON...")
            self.get_logger().info("="*60)
            
            agents_list = self._parse_plan_json(plan_json)
            
            if not agents_list:
                self.get_logger().warn(f"[Request-{request_id}] No valid agents extracted from plan")
                self.stats['failed'] += 1
                return False
            
            # Check for cancellation
            if self._cancel_requested:
                self.get_logger().warn(f"[Request-{request_id}] Cancelled after parsing")
                return False
            
            self.get_logger().info(f"[Request-{request_id}] Planning for {len(agents_list)} agents/formations")
            
            self.get_logger().info("="*60)
            self.get_logger().info(f"[Request-{request_id}] Starting sequential path planning...")
            self.get_logger().info("="*60)
            ...
            for agent_entry in agents_list:
                ...
                if self._cancel_requested:
                    self.get_logger().warn(f"[Request-{request_id}] Cancelled during agent planning loop")
                    return False
                ...
                self.get_logger().info(f"\n[Request-{request_id}] >>> Planning for {name} ({agent_type})...")
                ...
                if self._cancel_requested:
                    self.get_logger().warn(f"[Request-{request_id}] Cancelled after planning {name}")
                    return False
                
                if traj is None:
                    # Check if this was due to cancellation vs planning failure
                    if self._cancel_requested:
                        self.get_logger().warn(f"[Request-{request_id}] Planning cancelled for {name}")
                        return False
                    else:
                        self.get_logger().error(f"[Request-{request_id}] Planning failed for {name}, skipping...")
                        # continue  # Only continue if it's a genuine planning failure
                        return False
            ...
            if self._cancel_requested:
                self.get_logger().warn(f"[Request-{request_id}] Cancelled before publishing")
                return False
            
            if all_results:
                self.get_logger().info("="*60)
                self.get_logger().info(f"[Request-{request_id}] Publishing paths for {len(all_results)} agents")
                self.get_logger().info("="*60)
                ...
                # Check for cancellation before expensive image operation
                if self._cancel_requested:
                    self.get_logger().warn(f"[Request-{request_id}] Cancelled before saving image")
                    return False
                
                if self.save_image:
                    self._save_path_image(all_results)
                
                # Check for cancellation before expensive video operation
                if self._cancel_requested:
                    self.get_logger().warn(f"[Request-{request_id}] Cancelled before saving video")
                    return False
                ...
                self.get_logger().warn(f"[Request-{request_id}] No paths were successfully planned")
                self.stats['failed'] += 1
                return False
        
        except Exception as e:
            self.get_logger().error(f"[Request-{request_id}] Planning error: {e}")
            traceback.print_exc()
            self.stats['failed'] += 1
            return False
    ...
    def _plan_single_agent(self, request_id: int, name: str, agent, start: np.ndarray, goal: np.ndarray,
        ...
        try:
            ...
            def check_cancellation():
                """Returns True if planning should be cancelled"""
            ...
            self.get_logger().info(f"[Request-{request_id}] Starting planner for {name}...")
            ...
            self.get_logger().info(f"{'-'*10} Before Planning {'-'*10}")
            self.get_logger().info("**"*10)

            traj = planner.plan(start, goal, dynamic_obstacles)

            self.get_logger().info("**"*10)
            self.get_logger().info(f"{'-'*10} After Planning {'-'*10}")
            self.get_logger().info("**"*10)
            ...
            if traj is None:
                self.get_logger().error(f"[Request-{request_id}] Planner returned None for {name}")
                return None
            
            return traj
            
        except Exception as e:
            self.get_logger().error(f"[Request-{request_id}] Planning exception for {name}: {e}")
            traceback.print_exc()
            return None
    ...
    def _save_trajectories(self, all_results: list):
        ...
        for result in all_results:
            ...
            for i, robot_name in enumerate(robot_names):
                ...
                # ✅ Save CSV
                np.savetxt(
                    run_dir / f"{name}_{robot_name}.csv",
                    arr,
                    delimiter=",",
                    header="x,y,t",
                    comments=""
    ...
    def destroy_node(self):
        """Clean up resources when node is destroyed - SIMPLIFIED (NO THREADING)"""
        self.get_logger().info("Shutting down Path Planner Node...")
        
        # Cancel any ongoing planning
        if self._is_planning:
            self._cancel_requested = True
            self.get_logger().info("Cancelling ongoing planning operation...")
