...
LOG_DIR = (
    f"/home/{os.environ['USER']}/murogai_nav/src/MuRoGAI-Nav/"
    "path_plan/path_planner/path_planner/run_logs"
)
...
MAX_POSE_OFFSET  = 2.0   # global default; override per robot in RobotConfig
COORD_TIMEOUT    = 40.0
ODOM_STALE_WARN  = 4.0 #0.8
ODOM_STALE_LIMIT = 15.0 #2.0
WATCHDOG_PERIOD  = 2.0
...
ROBOT_CONFIG: dict[str, RobotConfig] = {
    ...
    "waffle": RobotConfig(
        csv_file="tb4_1.csv",
        kp_linear=1.42, kp_angular=0.4, ky=1.5,
        v_max=0.26, w_max=1.82, use_rsr=False,
    ),
    "firebird": RobotConfig(
        csv_file="waffle.csv",
        kp_linear=0.18, kp_angular=0.09, ky=1.2,
    ...
    def report_ready(self, robot_name, now_sec):
        with self._lock:
            ...
            if (all_ready or timed_out) and self._start_time is None:
                ...
                if missing:
                    print(f"[COORD] TIMEOUT after {self._timeout}s — "
...
class PathFollower(Node):
    def __init__(self, robot_name, coordinator):
        ...
        self._pose_offset_retries = 0
        self._last_odom_time      = 0.0
        self._last_odom_warn_time = 0.0
        self._last_cmd_time       = 0.0
        self._watchdog_warned     = False

        # CSV logging — timestamped file per robot per run
        os.makedirs(LOG_DIR, exist_ok=True)
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        odom_path = os.path.join(LOG_DIR, f"{robot_name}_odom_{ts}.csv")
        cmd_path  = os.path.join(LOG_DIR, f"{robot_name}_cmd_{ts}.csv")
        self._odom_file = open(odom_path, 'w', newline='')
        self._cmd_file  = open(cmd_path,  'w', newline='')
        self._odom_csv  = csv.writer(self._odom_file)
        self._cmd_csv   = csv.writer(self._cmd_file)
        self._odom_csv.writerow(['time_s', 'x', 'y', 'yaw_rad'])
        self._cmd_csv.writerow(['time_s', 'linear_x', 'angular_z'])
        self.get_logger().info(
            f"[LOG]  {self.robot}: odom → {odom_path}  cmd → {cmd_path}"
        )
        ...
        self.get_logger().info(
            f"[INIT] {self.robot}  sub_qos=SENSOR_DATA  pub_qos=RELIABLE  "
            f"kp_lin={self.cfg.kp_linear} kp_ang={self.cfg.kp_angular} "
            f"ky={self.cfg.ky}  v_max={self.cfg.v_max} w_max={self.cfg.w_max}"
        )

        self.create_subscription(
            Odometry, f'/{self.robot}/odom_world',
            self.odom_callback, QOS_ODOM_SUB)
    ...
    def odom_callback(self, msg):
        ...
        self._odom_csv.writerow([f"{self._last_odom_time:.6f}",
                                  f"{pos.x:.6f}",
                                  f"{pos.y:.6f}",
                                  f"{yaw:.6f}"])

    # ------------------------------------------------------------------
    def _log_cmd(self, cmd, now):
        self._cmd_csv.writerow([f"{now:.6f}",
                                  f"{cmd.linear.x:.6f}",
                                  f"{cmd.angular.z:.6f}"])
    ...
    def create_trajectory(self, shared_start_time):
        ...
        if offset > self.cfg.max_pose_offset:
            self._pose_offset_retries += 1
            if self._pose_offset_retries <= 5:
                self.get_logger().warn(
                    f"[TRAJ] {self.robot}: pose offset ({dx:.2f},{dy:.2f}) = "
                    f"{offset:.2f}m exceeds max_pose_offset="
                    f"{self.cfg.max_pose_offset}m — "
                    f"retry {self._pose_offset_retries}/5"
                )
                return
            else:
                self.get_logger().error(
                    f"[TRAJ] {self.robot}: pose offset {offset:.2f}m still "
                    f"exceeds max_pose_offset={self.cfg.max_pose_offset}m "
                    f"after 5 retries — proceeding anyway. "
                    f"Check odom or robot starting position."
                )
        ...
        if self.cfg.use_rsr:
            ...
            self._n_wp_ends = sum(
                1 for s in self.segments if s.get('is_wp_end', False))
            ...
            if dropped:
                self.get_logger().warn(
                    f"[TRAJ] {self.robot}: {dropped} CSV segments dropped "
                    f"(dt < 0.1s) — robot will clamp to first segment immediately"
                )
    ...
    def _watchdog(self):
        ...
        if gap > WATCHDOG_PERIOD and not self._watchdog_warned:
            ...
            seg_str = ("None" if seg is None
                       else f"[{seg['t0']:.1f},{seg['tf']:.1f}]")
            self.get_logger().error(
    ...
    def control_loop(self):
        ...
        # Odom staleness — tiered response
        ...
        if self._last_odom_time > 0 and odom_age > ODOM_STALE_WARN:
            if odom_age > ODOM_STALE_LIMIT:
                ...
                self._log_cmd(zero, now)
                return
            else:
                if now - self._last_odom_warn_time > 1.0:
                    self._last_odom_warn_time = now
                    self.get_logger().warn(
                        f"[ODOM] {self.robot}: odom slightly stale "
                        f"({odom_age:.2f}s) — continuing with last known pose"
                    )
        ...
        if not self.traj_created:
            ...
            if shared_start is not None:
                self._pending_start = shared_start
        ...
        # Clamp past any gap from dropped segments
...
def main():
    ...
    try:
        ...
        for node in nodes:
            node._odom_file.close()
            node._cmd_file.close()
            node.destroy_node()
        rclpy.shutdown()
