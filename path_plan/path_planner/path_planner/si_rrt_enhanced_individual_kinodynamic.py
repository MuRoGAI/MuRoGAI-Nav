#!/usr/bin/env python3
"""
si_rrt_enhanced_individual_kinodynamic.py  —  PATCHED (abort-aware)

Changes vs previous version
----------------------------
1. Kinematics.travel_time() derives speed from the agent object, not the
   global planner max_velocity (unchanged from prior patch).

2. SIRRT.plan() now accepts an optional  abort_flag  parameter:
       abort_flag : list([bool]) or None
   When abort_flag[0] is True the method returns None immediately.
   The check runs at the top of every ABORT_CHECK_INTERVAL-th main
   iteration AND at the top of every greedy-extend inner iteration.
   No threads are used — the check is fully cooperative.

   Callers that do not pass abort_flag get identical behaviour to the
   original (abort_flag defaults to None and all checks are skipped).
"""
...
# ── how often (iterations) to poll the abort flag ───────────────────
...
# ============================================================
# Numba-accelerated functions  (all unchanged)
# ============================================================
...
def _complement_intervals(
        blocked: List[Tuple[float, float]], T: float) -> List[SafeInterval]:
    blocked = _merge_intervals(
        [(max(0.0, s), min(T, e)) for s, e in blocked if e > s + 1e-12])
...
def _blocked_intervals_fixed_disc_vs_moving_disc_traj(
    ...
    if NUMBA_AVAILABLE:
        ...
        return [(float(result[i, 0]), float(result[i, 1]))
...
def ff_dist_sq(q1: np.ndarray, q2: np.ndarray,
               Nx: float, Ny: float, Nxy: float, Nr: int) -> float:
...
class IndividualAgent:
    ...
    def interpolate_q(self, q1: np.ndarray, q2: np.ndarray,
                      a: float) -> np.ndarray:
    ...
    def max_robot_displacement(self, q1: np.ndarray,
                               q2: np.ndarray) -> float:
        q1 = np.asarray(q1, dtype=float)
        q2 = np.asarray(q2, dtype=float)
        return math.hypot(float(q2[0] - q1[0]), float(q2[1] - q1[1]))
...
class FlexibleFormationAgent:
    def __init__(
        ...
        if isinstance(radius, (int, float)):
            self.radii = np.array([float(radius)] * self.Nr,
                                  dtype=np.float64)
        else:
            self.radii = np.array([float(r) for r in radius],
                                  dtype=np.float64)
            if len(self.radii) != self.Nr:
                raise ValueError(
                    f"radius length ({len(self.radii)}) != Nr ({self.Nr})")
        ...
        self._disc_cache: Dict[Tuple[float, ...],
    ...
    def _qkey(self, q: np.ndarray) -> Tuple[float, ...]:
        return tuple(np.round(np.asarray(q, dtype=float),
                              self._disc_cache_precision))
    ...
    def discs(self, q: np.ndarray) -> List[Tuple[np.ndarray, float]]:
        ...
        if cached is not None:
            return [(cached[i, :2].copy(), cached[i, 2])
        ...
        if NUMBA_AVAILABLE:
            ...
            return [(discs_array[i, :2].copy(), discs_array[i, 2])
                    for i in range(discs_array.shape[0])]
        ...
        return [(P[i, :].copy(), self.radii[i]) for i in range(self.Nr)]
    ...
    def robot_poses(self, q: np.ndarray) -> List[Tuple[float, float, float]]:
        ...
        if cached is not None:
            return [(cached[i, 0], cached[i, 1], cached[i, 2])
                    for i in range(cached.shape[0])]
        ...
        if NUMBA_AVAILABLE:
            ...
            return [(poses_array[i, 0], poses_array[i, 1], poses_array[i, 2])
                    for i in range(poses_array.shape[0])]

        xc, yc, th, sx, sy = (float(q[0]), float(q[1]), float(q[2]),
                               float(q[3]), float(q[4]))
        c, s = math.cos(th), math.sin(th)
        Rm = np.array([[c, -s], [s, c]], dtype=float)
        Sm = np.array([[sx, 0.0], [0.0, sy]], dtype=float)
        center = np.array([xc, yc], dtype=float)
        P = (Rm @ (Sm @ self.P_star.T)).T + center
        result = [(float(P[i, 0]), float(P[i, 1]), th)
                  for i in range(self.Nr)]
        if len(self._robot_poses_cache) > 500:
            self._robot_poses_cache.clear()
        self._robot_poses_cache[key] = np.array(result, dtype=float)
        return result
    ...
    def max_robot_displacement(self, q1: np.ndarray,
        ...
        q1 = np.asarray(q1, dtype=float)
        q2 = np.asarray(q2, dtype=float)
        if math.hypot(float(q2[0]-q1[0]), float(q2[1]-q1[1])) < 1e-6:
            return 0.0
        if NUMBA_AVAILABLE:
        ...
        md = 0.0
        for i in range(self.Nr):
            d = math.hypot(poses2[i][0]-poses1[i][0],
                           poses2[i][1]-poses1[i][1])
            if d > md:
                md = d
        return md
    ...
    @classmethod
    def _rs_like_proxy(cls, pose1, pose2,
                       lateral_weight=4.0, turn_weight=0.35):
        if NUMBA_AVAILABLE:
    ...
    def max_rs_like_distance(self, q1: np.ndarray,
                             q2: np.ndarray) -> float:
        q1    = np.asarray(q1, dtype=float)
        q2    = np.asarray(q2, dtype=float)
...
class SafeIntervalMap:
    ...
    @staticmethod
    def _disc_trajs_from_obstacle(
            obs: dict) -> List[List[Tuple[np.ndarray, float]]]:
        agent = obs["agent"]
        traj  = obs["trajectory"]
        if not traj or len(traj) < 2:
            return []
        ...
        M = len(discs0)
        per_disc: List[List[Tuple[np.ndarray, float]]] = \
            [[] for _ in range(M)]
        for (qk, tk, *_) in traj:
            ...
            if len(dlist) != M:
                raise RuntimeError(
                    "Obstacle disc count changed across trajectory.")
            for j in range(M):
                per_disc[j].append(
                    (np.asarray(dlist[j][0], dtype=float)[:2], tk))
        return per_disc
    ...
    def compute(
        ...
        for (p, r) in my_discs:
            if static_grid.disc_collides(float(p[0]), float(p[1]),
                                         float(r)):
                self._cache[k] = []
                return []
        ...
        for obs in dynamic_obstacles:
            ...
            if not per_disc_trajs:
                continue
            agent_obs  = obs["agent"]
            obs_discs0 = agent_obs.discs(
                np.asarray(obs["trajectory"][0][0], dtype=float))
            ...
            for (p_fixed, r_fixed) in my_discs:
                ...
                for j, traj_j in enumerate(per_disc_trajs):
                    ...
                    blocked.extend(
                        _blocked_intervals_fixed_disc_vs_moving_disc_traj(
                            p_fixed, Rsum, traj_j, T))
...
def interpolate_traj_q(traj: List[Tuple[np.ndarray, float]],
    ...
    for i in range(len(traj) - 1):
        ...
        if t0 <= t <= t1:
            ...
            if q.shape[0] >= 3:
                th0, th1v = float(q0[2]), float(q1[2])
                dth  = (th1v - th0 + math.pi) % (2.0*math.pi) - math.pi
                q[2] = th0 + a * dth
            if q.shape[0] >= 5:
...
# ============================================================
# Kinematics  (travel-time patch — unchanged from prior version)
# ============================================================
...
class Kinematics:
    ...
    def travel_time(self, agent, q_from: np.ndarray,
                    q_to: np.ndarray) -> float:
        ...
        if is_formation:
            if hasattr(agent, 'v_max_list') and hasattr(agent, 'robot_poses'):
                try:
                    ...
                    t_needed = 0.0
                    for i, ((x1,y1,_),(x2,y2,_)) in enumerate(
                            zip(poses1, poses2)):
...
# ============================================================
# SIRRT planner  —  abort_flag added to plan()
# ============================================================
...
class SIRRT:
        ...
        if use_kinodynamic and hasattr(agent_model, 'P_star'):
            ...
            if is_heterogeneous and HETEROGENEOUS_KINO_AVAILABLE:
                try:
                    ...
                    self.kino_steerer    = HeterogeneousKinodynamicFormationSteering(
                        P_star=agent_model.P_star, **kino_defaults)
                    self.use_kinodynamic = True
                    if self.debug:
                        print("✓ Heterogeneous formation steering enabled")
                except Exception as e:
                    if self.debug:
                        print(f"⚠ Failed to init heterogeneous steering: {e}")
                    is_heterogeneous = False

            if not is_heterogeneous or not HETEROGENEOUS_KINO_AVAILABLE:
                try:
                    try:
                        from kinodynamic_steering_with_full_output \
                            import KinodynamicFormationSteering
                        KINO_AVAILABLE = True
                    except ImportError:
            ...
            if INDIVIDUAL_KINO_AVAILABLE:
                ...
                if agent_type == 'diff-drive':
                    individual_kino_params['v_min'] = \
                        -self.kin._agent_effective_vmax(agent_model) * 0.5
                    individual_kino_params['omega_max'] = \
                        getattr(agent_model, 'omega_max', 2.0)
                    individual_kino_params['alpha_max'] = \
                        getattr(agent_model, 'alpha_max', 4.0)
                    individual_kino_params['max_time'] = 5.0
                if kinodynamic_params:
                    individual_kino_params.update(kinodynamic_params)

                try:
                    self.individual_kino_steerer = create_steering_for_agent(
                        agent_type, **individual_kino_params)
                    self.use_kinodynamic = True
                    if self.debug:
                        print(f"✓ Individual kinodynamic steering ({agent_type})")
                except Exception as e:
                    if self.debug:
                        print(f"⚠ Failed to init individual steering: {e}")
    ...
    def _reset_per_plan_caches(self):
        ...
        if hasattr(self.agent, "clear_cache") and \
                callable(getattr(self.agent, "clear_cache")):
    ...
    def _rebuild_kdtree(self):
        if not KDTREE_AVAILABLE or len(self.V) < 20:
            ...
            return
        self._V_xy    = np.array(
            [self.agent.centroid_xy(v.q) for v in self.V], dtype=np.float64)
        self._kdtree  = cKDTree(self._V_xy)
        self._kdtree_size = len(self.V)

    def _maybe_rebuild_kdtree(self):
        if KDTREE_AVAILABLE and \
                len(self.V) - self._kdtree_size >= self._kdtree_rebuild_threshold:
    ...
    def debug_check_same_q_multiple_SIs(self) -> bool:
        ...
        print(f"[DEBUG] Any config with >=2 distinct SIs? {any_multi}")
        return any_multi
    ...
    def debug_dump_vertices(self, top_k: int = 30) -> None:
        ...
        print(f"[DEBUG] qkeys with >1v : {len(multi)}")
        for idx, (k, vs) in enumerate(multi[:top_k]):
            ...
            print(f"\n[DEBUG] qkey #{idx+1} = {k}  "
                  f"vertices={len(vs)}  distinct_SIs={len(si_groups)}")
            for sik, g in sorted(si_groups.items(), key=lambda x: x[0]):
                times = sorted(vv.t for vv in g)
                med   = times[len(times)//2]
                print(f"  SI={sik}  count={len(g)}  "
                      f"t(min/med/max)={min(times):.3f}/{med:.3f}/{max(times):.3f}")
    ...
    def debug_check_near_duplicate_SIs(self, eps: float = 1e-8) -> None:
        ...
        for k, sis in by_q.items():
            ...
            for a, b in zip(sis_sorted[:-1], sis_sorted[1:]):
                if (abs(float(a.low)  - float(b.low))  < eps and
                        abs(float(a.high) - float(b.high)) < eps and
                        (a.low != b.low or a.high != b.high)):
                    print(f"  qkey={k}  "
                          f"({a.low:.12f},{a.high:.12f}) vs "
                          f"({b.low:.12f},{b.high:.12f})")
                    break
    ...
    def _steer(self, q_from: np.ndarray, q_to: np.ndarray,
        ...
        if (self.use_kinodynamic and self.kino_steerer is not None
                and q_from.shape[0] >= 5):
            try:
                q_new, psi_new = self.kino_steerer.steer(
                    q_from, q_to, psi_from)
                if q_new is not None:
                    return q_new, psi_new
            except Exception as e:
                if self.debug:
                    print(f"⚠ Kinodynamic steering failed: {e}, fallback")

        if (q_from.shape[0] < 5 or
                not hasattr(self.agent, "max_robot_displacement")):
            if (self.use_kinodynamic and
                    self.individual_kino_steerer is not None):
                try:
                    q_new, control_traj = \
                        self.individual_kino_steerer.steer(
                            q_from, q_to, step_size=self.d_max)
                    return q_new, control_traj
                except Exception as e:
                    if self.debug:
                        print(f"⚠ Individual kino steering failed: {e}")
            p = self.agent.centroid_xy(q_from)
            g = self.agent.centroid_xy(q_to)
            L = (math.sqrt(_nb_dist_sq_xy(p[0], p[1], g[0], g[1]))
                 if NUMBA_AVAILABLE else float(np.linalg.norm(g - p)))
            if L <= self.d_max + 1e-12:
                return q_to.copy(), None
            return self.agent.interpolate_q(
                q_from, q_to, self.d_max / (L + 1e-12)), None

        d_eff = float(self.agent.max_robot_displacement(q_from, q_to))
        if d_eff <= self.d_max + 1e-12:
            return q_to.copy(), (psi_from.copy() if psi_from is not None
                                 else None)

        a = max(0.0, min(1.0, self.d_max / (d_eff + 1e-12)))

        xc, yc, th, sx, sy = (float(q_from[0]), float(q_from[1]),
                               float(q_from[2]), float(q_from[3]),
                               float(q_from[4]))
        xg, yg, thg, sxg, syg = (float(q_to[0]), float(q_to[1]),
                                   float(q_to[2]), float(q_to[3]),
                                   float(q_to[4]))
        ...
        max_dtheta = kappa_max * self.d_max
        ...
        q_new        = np.array([xc_new, yc_new, th_new, sx_new, sy_new],
                                dtype=float)
        psi_fallback = None
    ...
    def _static_ok(self, q: np.ndarray) -> bool:
        ...
        for (p, r) in self.agent.discs(q):
            if self.static_grid.disc_collides(float(p[0]), float(p[1]),
                                              float(r)):
                ok = False; break
        self._static_ok_cache[k] = ok
        return ok

    def _edge_samples_count(self, q_from: np.ndarray, q_to: np.ndarray,
                            step: float, min_n: int = 3) -> int:
        p0 = self.agent.centroid_xy(q_from)
        p1 = self.agent.centroid_xy(q_to)
        L  = (math.sqrt(_nb_dist_sq_xy(p0[0], p0[1], p1[0], p1[1]))
              if NUMBA_AVAILABLE else float(np.linalg.norm(p1 - p0)))
        if step <= 1e-12:
            return 14
        return max(min_n, int(L / step))

    def _edge_static_ok(self, q_from: np.ndarray,
                        q_to: np.ndarray) -> bool:
        step = 0.5 * max(self._agent_radius, 1e-6)
        n    = self._edge_samples_count(q_from, q_to, step=step, min_n=3)
        for k in range(n + 1):
            q_mid = self.agent.interpolate_q(q_from, q_to, k / n)
            if not self._static_ok(q_mid):
                return False
        return True

    def _dynamic_ok(self, q: np.ndarray, t: float,
                    dynamic_obstacles: List[dict]) -> bool:
        ...
        for obs in dynamic_obstacles:
            ...
            for (p, r) in my_discs:
                ...
                for (op, orad) in other_discs:
                    if NUMBA_AVAILABLE:
                        if _nb_discs_overlap(px, py, float(r),
                                             float(op[0]), float(op[1]),
                                             float(orad)):
                            return False
                    else:
                        if float(np.linalg.norm(p - op)) < (float(r) +
                                                              float(orad)):
                            return False
        return True

    def _edge_dynamic_ok(self, q_from, q_to, t_depart, travel,
                         dynamic_obstacles):
        if not dynamic_obstacles:
            return True
        step = 0.5 * max(self._agent_radius, 1e-6)
    ...
    def _earliest_arrival(self, v_from, q_to, si_to, dynamic_obstacles):
        ...
        if dep_low >= dep_high - 1e-9:
            return None
        t_arr = dep_low + travel
        if t_arr > self.T:
            return None
        if not self._edge_dynamic_ok(v_from.q, q_to, dep_low, travel,
                                     dynamic_obstacles):
            return None
        return t_arr

    # ------------------------------------------------------------------
    # plan()  —  abort_flag parameter added here
    # ------------------------------------------------------------------
    ...
    def plan(self, q_start, q_goal, dynamic_obstacles,
        ...
        Parameters
        ----------
        q_start, q_goal : array-like
        dynamic_obstacles : list[dict]
        abort_flag : list([bool]) or None
            Pass [False].  Set abort_flag[0] = True at any time (from the
            file-watcher loop) to make plan() return None immediately.
            Checked every ABORT_CHECK_INTERVAL iterations of the main loop
            and at the start of every greedy-extend inner step.
            If None, no abort checks are performed (original behaviour).
        """
        ...
        si_start_list = self.si_map.compute(
            q_start, self.agent, self.static_grid, dynamic_obstacles, self.T)
        ...
        v0 = Vertex(q=q_start.copy(), t=0.0, si=si0,
                    parent=None, psi=psi_start)
        self.V = [v0]
        ...
        best_goal_t: float            = float("inf")
        q_goal_xy = _as_xy(q_goal)

        for _it in range(self.max_iter):

            # ── ABORT CHECK (main loop) ──────────────────────────────
            if (abort_flag is not None
                    and _it % ABORT_CHECK_INTERVAL == 0
                    and _it > 0
                    and abort_flag[0]):
                print(f"  [ABORT] abort_flag set at iteration {_it} "
                      f"— returning None from plan().")
                return None
            ...
            q_new, psi_new = self._steer(v_near.q, q_rand,
                                          psi_from=v_near.psi)
            ...
            si_list = self.si_map.compute(
                q_new, self.agent, self.static_grid, dynamic_obstacles, self.T)
            if not si_list:
                continue
            ...
            for si_to in si_list:
                ...
                for v in neighbors:
                    if not self._edge_static_ok(v.q, q_new):
                        continue
                    t_arr = self._earliest_arrival(
                        v, q_new, si_to, dynamic_obstacles)
                    if t_arr is None:
                        continue
                ...
                v_new = Vertex(q=q_new.copy(), t=best_arr, si=si_to,
                               parent=best_parent, psi=psi_new,
                ...
                if not dynamic_obstacles:
                    ...
                    max_extend = int(max(
                        5, min(80, math.ceil(
                            self._nn_dist(q_cur, q_goal)
                            / max(self.d_max, 1e-6)) + 5)))

                    for _j in range(max_extend):

                        # ── ABORT CHECK (greedy extend) ──────────────
                        if abort_flag is not None and abort_flag[0]:
                            print(f"  [ABORT] abort_flag set in greedy "
                                  f"extend (it={_it}, j={_j}) "
                                  f"— returning None from plan().")
                            return None
                        ...
                        q_next, psi_next = self._steer(
                            q_cur, q_goal, psi_from=psi_cur)
                        control_traj_next = None
                        if (psi_next is not None and
                                not isinstance(psi_next, np.ndarray)):
                        ...
                        travel = self.kin.travel_time(
                            self.agent, q_cur, q_next)
                        t_next = t_cur + travel
                        if t_next > self.T:
                            break

                        si_list_next = self.si_map.compute(
                            q_next, self.agent, self.static_grid, [], self.T)
                        if not si_list_next:
                            break
                        si_next = si_list_next[0]
                        for si_cand in si_list_next:
                            if si_cand.low - 1e-9 <= t_next \
                                    < si_cand.high + 1e-9:
                                si_next = si_cand
                                break

                        v_next = Vertex(
                            q=q_next.copy(), t=float(t_next),
                            si=si_next, parent=v_cur,
                            psi=psi_next,
                            control_traj=control_traj_next)
                        q_cur   = q_next
                        psi_cur = psi_next
                        ...
                        if self._nn_dist(q_cur, q_goal) \
                                <= self.d_max + 1e-9:
                            q_next2, _ = self._steer(q_cur, q_goal)
                            if q_next2 is not None and \
                                    self._edge_static_ok(q_cur, q_next2):
                                travel2 = self.kin.travel_time(
                                    self.agent, q_cur, q_goal)
                                t_goal  = t_cur + travel2
                                if t_goal <= self.T:
                                    si_goal_list = self.si_map.compute(
                                        q_goal, self.agent,
                                        self.static_grid, [], self.T)
                                    if si_goal_list:
                                        si_g = si_goal_list[0]
                    ...
                    close_enough = (
                        _nb_dist_sq_xy(
                            q_new_xy[0], q_new_xy[1],
                            q_goal_xy[0], q_goal_xy[1]) <= 0.49
                        if NUMBA_AVAILABLE
                        else np.linalg.norm(q_new_xy - q_goal_xy) <= 0.7
                    )

                    if close_enough and \
                            self._edge_static_ok(q_new, q_goal):
                        si_goal_list = self.si_map.compute(
                            q_goal, self.agent, self.static_grid,
                            dynamic_obstacles, self.T)
                        for si_g in si_goal_list:
                            t_goal = self._earliest_arrival(
                                v_new, q_goal, si_g, dynamic_obstacles)
                            if t_goal is not None and \
                                    t_goal < best_goal_t:
                                best_goal_t = t_goal
                                best_goal   = Vertex(
                                    q=q_goal.copy(),
            ...
            if (best_goal is not None and _it > 500
                    and best_goal_t < self.T * 0.5):
                break
        ...
        while v is not None:
            ...
            if v.psi is not None:
                ...
                elif (INDIVIDUAL_KINO_AVAILABLE and
                      isinstance(v.psi, ControlTrajectory)):
            ...
            out.append((v.q.copy(), float(v.t),
                        psi_to_save, control_traj_to_save))
            v = v.parent
        out.reverse()
