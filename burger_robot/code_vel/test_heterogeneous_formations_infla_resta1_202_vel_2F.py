...
hetero_form = HeterogeneousFormationAgent(
    ...
    # BUG FIX: explicitly set per-robot velocity limits.
    ...
    v_max=[0.16, 0.16,0.16],       # per-robot linear velocity limit [m/s]
    omega_max=[1.7, 1.7, 1.7],   # per-robot angular velocity [rad/s]
    a_max=[0.9, 0.9, 2.5],          # per-robot linear accel [m/s^2]
    alpha_max=[2.0, 2.0, 2.0],      # per-robot angular accel [rad/s^2]
    sx_range=(0.8, 1.3),
    sy_range=(0.8, 1.3),
    radius=[0.25, 0.25, 0.25],  # per-robot radius [m]
)
...
hetero_form1 = HeterogeneousFormationAgent(
    ...
    # Without this, v_max defaults to 1.0 -> v_max_list=[1.0,1.0,1.0]
    # and travel_time() divides by 1.0 instead of the real hardware limit.
    v_max=[0.28, 0.2, 0.8],       # per-robot linear velocity limit [m/s]
    omega_max=[1.7, 1.7, 0.0],   # per-robot angular velocity [rad/s]
    a_max=[0.9, 2.5, 2.5],          # per-robot linear accel [m/s^2]
...
agents = [
     ...
     np.array([1.71, 1.33, -1.57, 1.0, 1.0]),
...
colors = {
    ...
    "HeteroForm1": "tab:orange",
    "HeteroForm2": "tab:brown",
    "Holo2": "tab:red",
}
