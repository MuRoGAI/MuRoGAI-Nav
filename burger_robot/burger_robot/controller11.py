...
#                 This is the DEFAULT assumed type. The actual type from the
#                 incoming RobotTrajectoryArray message always takes priority
#                 and overrides this at runtime.
#
#  use_xy_only  : (diff-drive only) True  → Kanayama x,y spline, theta ignored
#                                   False → full x,y,theta spline tracking
#
#  max_lin_y    : (holonomic only)  lateral velocity limit [m/s]
#                 Set to 0.0 for diff-drive (unused but harmless).
# ═════════════════════════════════════════════════════════════════════════════
@dataclass
class RobotConfig:
    robot_type:  str   = "diff-drive"   # default; overridden by message at runtime
    kp_linear:   float = 1.5
    kp_angular:  float = 2.5
    ky:          float = 3.5
    v_max:       float = 0.22           # max forward (diff) / x (holo) speed [m/s]
    w_max:       float = 2.84           # max yaw rate [rad/s]
    max_lin_y:   float = 0.0            # max lateral speed [m/s] — holonomic only
    use_xy_only: bool  = False          # diff-drive only: ignore theta from planner
...
ROBOT_CONFIG: dict[str, RobotConfig] = {
    ...
    "delivery_bot3": RobotConfig(
        ...
        v_max=0.25, w_max=0.4,
        use_xy_only=True,   # Kanayama, theta ignored
    ),
    # ── Holonomic robots ───────────────────────────────────────────────────
    "cleaning_bot": RobotConfig(
        robot_type="holonomic",
        kp_linear=2.0, kp_angular=1.8, ky=0.0,   # ky unused for holonomic
        v_max=0.25, w_max=0.4, max_lin_y=0.25,
    ),
...
class PathFollower(Node):
    ...
    def __init__(self, robot_name: str):
        ...
        # Runtime robot type — overridden by the incoming trajectory message.
        ...
        self.get_logger().info(
            ...
            f"max_lin_y={self.cfg.max_lin_y}  "
    ...
    def trajectory_array_callback(self, msg: RobotTrajectoryArray):
        ...
        for robot_traj in msg.robot_trajectories:
            ...
            # ── Robot type comes from the MESSAGE (authoritative) ──────────
            robot_type = robot_traj.robot_type   # "diff-drive" or "holonomic"
            ...
            if robot_type == "diff-drive":
                ...
                x_list     = traj.x
                y_list     = traj.y
                theta_list = traj.theta   # always present for diff-drive
                ...
                x_list     = traj.x
                y_list     = traj.y
                theta_list = None         # holonomic planner has no heading column
            ...
            # ── Build segments ─────────────────────────────────────────────
            # Holonomic: always xy-only (moves in any direction, no heading constraint)
            # Diff-drive: use_xy_only flag chooses Kanayama vs full theta tracking
            if robot_type == "holonomic":
                segments, duration = build_xy_spline(time_list, x_list, y_list)
                mode_label = "HOLONOMIC-XY"
            elif self.cfg.use_xy_only:
                segments, duration = build_xy_spline(time_list, x_list, y_list)
                mode_label = "DIFF-XY-KANAYAMA"
            else:
                segments, duration = build_spline(
                    time_list, x_list, y_list, theta_list)
                mode_label = "DIFF-SPLINE"
            ...
            # ── Atomic state update ────────────────────────────────────────
            with self._lock:
                self.robot_type     = robot_type   # runtime override from message
            ...
            self.get_logger().info(
                f"[TRAJ] {self.robot}: {len(segments)} segments  "
                f"total={duration:.2f}s  type={robot_type}  mode={mode_label}"
            )
    ...
    def control_loop(self):
        ...
        if t > last['tf']:
            ...
            if self.robot_type == "holonomic":
                # Holonomic SEEK: drive directly in world-frame x,y
                dx_g = xd_g - x
                dy_g = yd_g - y
                cmd.linear.x  = float(np.clip(cfg.kp_linear * dx_g, -cfg.v_max,    cfg.v_max))
                cmd.linear.y  = float(np.clip(cfg.kp_linear * dy_g, -cfg.max_lin_y, cfg.max_lin_y))
                cmd.angular.z = 0.0   # no yaw target for holonomic seek
            else:
                # Diff-drive SEEK: rotate toward goal then drive
    ...
    def _control_holonomic(self, t, seg, now):
        """
        Holonomic controller — world-frame P + feedforward.

        A holonomic robot can move in any direction independently of its
        heading, so we command vx, vy directly in the world frame and
        control yaw separately if needed.

        Control law:
            vx_cmd = xd_dot + kp_linear * (xd - x)
            vy_cmd = yd_dot + kp_linear * (yd - y)
            w_cmd  = 0   (no yaw target from planner for holonomic)

        The feedforward (xd_dot, yd_dot) keeps the robot on the time-
        parameterised spline; the P term corrects accumulated error.
        """
        xd, xd_dot = eval_cubic(t, seg['cx'])
        yd, yd_dot = eval_cubic(t, seg['cy'])
        ...
        vx_cmd = xd_dot + cfg.kp_linear * dx
        vy_cmd = yd_dot + cfg.kp_linear * dy

        cmd = Twist()
        cmd.linear.x  = float(np.clip(vx_cmd, -cfg.v_max,    cfg.v_max))
        cmd.linear.y  = float(np.clip(vy_cmd, -cfg.max_lin_y, cfg.max_lin_y))
        cmd.angular.z = 0.0   # holonomic path has no yaw requirement
        self.cmd_pub.publish(cmd)
        ...
        if now - self.last_debug_time > 0.5:
            ...
            self.get_logger().info(
                f"[CTRL] {self.robot} HOLO | t={t:.2f}s "
                f"seg=[{seg['t0']:.1f},{seg['tf']:.1f}] | "
                f"des=({xd:.2f},{yd:.2f}) "
                f"act=({x:.2f},{y:.2f}) | "
