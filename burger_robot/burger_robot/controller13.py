...
class RobotConfig:
    ...
    # Always True — BEST_EFFORT removes ACK/NACK overhead on odom.
    # With 4 robots at 50 Hz each = 200 msgs/sec; reliable QoS makes
    # DDS retransmit every dropped packet which compounds under load.
    best_effort_qos: bool = True
...
ROBOT_CONFIG: dict[str, RobotConfig] = {
    ...
    # "cleaning_bot1": RobotConfig(
    #     robot_name  = "cleaning_bot1",
    #     robot_type  = "holonomic",
    #     kp_linear   = 32.3,
    #     kp_angular  = 27.6,
    #     kd_linear   = 3.53,
...
class PathFollower(Node):
    def __init__(self, robot_name: str, coordinator: Coordinator):
        ...
        # ── QoS ──────────────────────────────────────────────────────────────
        # BEST_EFFORT for odom on all robots — removes ACK/NACK round-trips
        # that compound badly when 4 robots x 50 Hz = 200 msgs/sec on DDS.
        ...
        self.create_subscription(
            ...
            QOS_RELIABLE       # trajectory must be reliable — sent once
        )
    ...
    def _log_waypoint_crossings(self, t: float):
        """
        Fires once per segment when trajectory time passes seg["tf"].

        Time error = how many seconds late the robot is spatially.

            spatial_time_error = position_error / reference_speed

        position_error  -- straight-line distance between actual pose and
                           the desired pose at seg["tf"].
        reference_speed -- max tracking speed for this robot type:
                             diff-drive : max_lin_x
                             holonomic  : sqrt(max_lin_x^2 + max_lin_y^2)

        The old value (t - seg["tf"]) was just control-loop timer jitter
        (~0.001-0.01 s) and said nothing about the robot's actual lateness.
        """
        ...
        for i, seg in enumerate(self.segments):
            ...
            # Reference speed: planned tracking speed for this robot type.
            # Holonomic diagonal = sqrt(vx_max^2 + vy_max^2).
            if self.cfg.max_lin_y > 1e-6:
                ref_speed = math.hypot(self.cfg.max_lin_x, self.cfg.max_lin_y)
            else:
                ref_speed = self.cfg.max_lin_x
            ref_speed = max(ref_speed, 1e-3)  # guard against zero config

            # Seconds the robot is behind schedule at this waypoint.
            # If pos_err ~ 0 the robot is on time -> time_error ~ 0.
            spatial_time_error = pos_err / ref_speed

            self.get_logger().info(
                f"\n========== [{self.robot}] WAYPOINT {i+1}/{len(self.segments)} "
                f"==========\n"
                f"  Time error    : +{spatial_time_error:.4f} s  "
                f"({pos_err:.4f} m / {ref_speed:.3f} m/s)\n"
    ...
    def _control_diff_drive(self, t: float, seg: dict, now: float):
        ...
        if now - self.last_debug_time > 0.5:
            ...
            self.get_logger().info(
                ...
                f"des=({xd:.2f},{yd:.2f},th={math.degrees(desired_theta):.1f}deg) "
                f"act=({x:.2f},{y:.2f},{math.degrees(yaw):.1f}deg) | "
