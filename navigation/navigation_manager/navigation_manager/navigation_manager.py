...
class NavigationManager(Node):
    def __init__(self):
        ...
        self.system_prompt = (
            ...
            "   When obstacles are present in multiple directions around the formation goal, prefer adjusting the formation_yaw to orient the formation toward the largest available free space, and if clearance is still insufficient after reorientation, increase the desired_radius to push all robots further out into collision-free space. "
            ...
            "   Each robot in a formation must maintain a safe clearance such that no robot collides with any object at the formation centroid, nor with any obstacle outside the formation boundary. Use the robot radius from the robot configuration data to compute minimum safe separation distances and verify all goal positions are collision-free by ensuring the distance between each robot position and any object center is greater than (object_half_size + robot_radius). "
            "   Robot configuration data contains the radius for each robot - use it to enforce collision-free goal placement by ensuring no robot goal position overlaps with any object footprint, and no robot position is placed within (object_half_size + robot_radius) of any object center. When the desired_radius appears too large due to surrounding obstacles, adjust the formation_yaw to rotate the formation so that all robots fit within the available free space without colliding with any object. "
            "   For a formation goal, the formation centroid may be placed directly at the object position, but the desired_radius must be calculated such that every robot in the formation clears both the goal object and all surrounding obstacles, accounting for each robot's individual radius from the configuration data, verified using: distance between each robot position and object center must be greater than (object_half_size + robot_radius). If a collision is detected, reduce desired_radius incrementally until all robots are collision-free, or adjust formation_yaw to reorient the formation into free space. The desired_radius must remain the minimum viable value - neither too small to cause overlap nor unnecessarily large. "            # "   Position of robot should be within 2 meters PROXIMITY to the object and NOT directly on the object. "
